...
kubectl wait --for=condition=Available deployments/strimzi-cluster-operator  --timeout=120s -n $NAMESPACE
echo ">>> Strimzi operator ready"
kubectl wait --for=condition=Available deployments/debezium-operator  --timeout=120s -n $NAMESPACE
...
kubectl wait --for=condition=Available deployments/mongo --timeout=120s -n $NAMESPACE
...
kubectl wait --for=condition=Ready pods/tooling --timeout=120s -n $NAMESPACE
echo ">>> Tooling ready"
## Kafka
echo ">>> Waiting  for Kafka"
kubectl wait --for=condition=Ready kafkas/dbz-kafka --timeout=120s -n $NAMESPACE
echo ">>> Kafka ready"